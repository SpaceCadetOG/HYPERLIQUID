...
func runOnce(client *hyperliquid.Client, trk *inplay.Tracker, store *status.Store, tf string, candlesN int, topN int, engineURL string) {
	...
	rows, grades := sideRows(client, trk, tf, candlesN, "long", now)
	entries := filterDisplayInPlay(trk.Entries())
	store.SetSnap(status.Snapshot{Generated: now, Exchange: "hyperliquid (LONGS)", Active: sessions.ActiveSessionLabels(now), Rows: rows, Conf: grades, InPlay: entries, Engine: fetchEngineSnapshot(engineURL)})

	fmt.Println(market.FormatHeader("hyperliquid (LONGS)", sessions.ActiveSessionLabels(now)))
	fmt.Println("Symbol       | Score  | DayUTC% | UTC4h%  | UTC1h%  | Δ%(24h) | Vol($)  | OI($)   | Funding(%) | OpenUTC |     Last")
	fmt.Println("-------------+--------+---------+---------+---------+----------+---------+---------+------------+---------+---------")
	printRows(rows, grades, topN)
	printInPlay("LONG", entries)
}
...
func buildEligible(rows []market.Scored, side string, entryBySymbol map[string]inplay.Entry) ([]market.Scored, map[string]string) {
	...
	for _, row := range rows {
		...
		if !ok && !(hasEntry && inplay.EarlyLongAdmissionFromShortLeader(entry, earlyLongMin)) {
			continue
		}
		if hasEntry && entry.LongDemotionFlag {
			continue
		}
